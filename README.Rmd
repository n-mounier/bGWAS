...
devtools::check(pkg = ".", manual = TRUE, check_dir = "../bGWAS.check")
system("mv ../bGWAS.check/bGWAS.Rcheck/bGWAS-manual.pdf doc/")
# 
...
bGWAS is an R-package to perform a Bayesian GWAS, using summary statistics from a conventional GWAS as input. Briefly, it compares the observed Z-scores for a focal phenotype to prior effects. These prior effects are directly estimated from publicly available GWASs (currently, a set of 38 studies, last update 20-08-2019 - hereinafter referred to as "prior GWASs"). Only prior GWASs having a significant causal effect on the focal phenotype, identified using a multivariate Mendelian Randomization (MR) approach, are used to calculate the prior effects. Causal effects are estimated masking the focal chromosome to ensure independence.          
...
<ul>   SNP-identifier:  `rs` or `rsid`, `snp`, `snpid`, `rnpid`                       Alternate allele:  `a1` or `alt`, `alts`    
...
<ul><ul>   *ZMatrix_MR.csv.gz*: Z-scores (strong instruments only) used for multivariate MR,   
...
<font color="red">**Problem with switch drive, download link will be added soon!**</font>
...
Analysis using all the 38 prior GWASs available, for a conventional GWAS containing ~7M SNPs in common with the prior studies ~ 25 minutes (see complete Lifespan Analysis [here](doc/Lifespan_Analysis.md)).
