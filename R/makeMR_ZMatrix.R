...
makeMR_ZMatrix <- function(prior_studies=NULL, GWAS,
  ...
  #   tmp = paste0(paste0(colnames(ZMatrix[,-c(1:5)])[!StudiesToKeep], collapse=" - "), " : removed (no strong instrument in common) \n")
  #   if(save_files){
  ...
  # }
  # Log = update_log(Log, tmp, verbose)
  # tmp = paste0(ncol(ZMatrix)-6, " studies left after combining with conventional GWAS \n")
  # Log = update_log(Log, tmp, verbose)
  ...
  Log = update_log(Log, tmp, verbose)
  tmp = paste0(ncol(ZMatrix)-5, " studies left after pruning \n")
  Log = update_log(Log, tmp, verbose)
