...
These variants (and the ones in a 100kb window) can be further investigated using the [GWAS Catalog](https://www.ebi.ac.uk/gwas/). R2 estimates in EUR population from [LDlink](https://ldlink.nci.nih.gov/) are used to keep only SNPs in LD (R2>0.1) with the variant identified. SNP-trait associations p-values below 5e-8 are reported below (for SNPs in LD, p-values adjusted for correlation are used).


```{r auto_newhits, echo=T, results='asis'}
...
Interestingly, we can see that a few loci identified have been associated with some of the risk factors used to create the prior in more recent studies. Variants in LD with the variants identified near ZNF318 and ZC3HC1 have been associated with CAD and blood pressure, and a variant in LD with the variant identified near GADD45G has been associated with educational attainment. The other loci have not been associated with any of the risk factors.
