...
Other = system.file("Data/", package="bGWAS")
...
#### Z_matrices ####
test_that("ZMatrices files not in folder", {
  expect_error( bGWAS( name="aaa", GWAS=SmallGWAS_Timmers2019, Z_matrices = Other, verbose=F),
                regexp = "No \"ZMatrix_Full.csv.gz\" file in specified Z_matrices folder")
})


test_that("ZMatrices folder do not exist", {
  expect_error( bGWAS( name="aaa", GWAS=SmallGWAS_Timmers2019, Z_matrices = "~/Data/ZMat", verbose=F),
                regexp = "Z_matrices : the folder do not exist")
})
...
test_that("MR_threshold out-of-range", {
  expect_error( bGWAS( name="aaa", GWAS=SmallGWAS_Timmers2019, Z_matrices = ZMats, 
                ...
                regexp = "MR_threshold : superior to the threshold limit")
})
